...
import carele.buffers as buffers
...
class DQN:
    def __init__(self, config):
        ...
        self.env_type = config['env']['name_']
        self.buffer_name = config['agent']['buffer_name']
        self.action_space_num = config['env']['action_space']

        # Create Buffer
        self.Buffer = getattr(buffers, config['agent']['buffer_name'])
        self.buffer = self.Buffer(config['agent']['buffer_size'])

        # Create Model for Learning
        Net = getattr(models, config['policy']['network']['class'])
        self.current_model = Net(config)
        self.target_model = Net(config)
        self.device = T.device("cuda" if T.cuda.is_available() else "cpu")
        self.current_model.to(self.device)
        self.target_model.to(self.device)
        self.optimizer = T.optim.Adam(self.current_model.parameters(), lr=config['policy']['network']['learning_rate'])
    ...
    def learn(self, config):
        if config['agent']['buffer_name'] == 'PERBuffer':
            tree_index, minibatch, weights = self.buffer.sample(config['agent']['mini_batch'])
            tree_index = T.LongTensor(tree_index).to(self.device)
            weights = T.FloatTensor(weights).to(self.device)
        elif config['agent']['buffer_name'] == 'Simple_Buffer':
            minibatch = self.buffer.sample(config['agent']['mini_batch'])
        ...
        expected_q_value = reward + config['agent']['gamma'] * next_q_value * (1 - done)
